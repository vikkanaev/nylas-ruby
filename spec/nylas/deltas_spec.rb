...
describe NylasV3::Deltas do
  ...
  it "safely inflates an account.running event" do
    ...
    expect(delta.model).to be_a NylasV3::Account
  ...
  it "parses stream data from multiple changes" do
    ...
    expect(message_delta.model).to be_a(NylasV3::Message)
    ...
    expect(event_delta.model).to be_a(NylasV3::Event)
  ...
  it "parses deltas if `attributes` is not present" do
    ...
    expect(delta.model).to be_a(NylasV3::Event)
    expect(delta.attributes.to_h).to eq(
      id: "some-id",
      object: "event"
    )
